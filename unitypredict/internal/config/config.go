...
type Config struct {
	...
	DevUnitypredictToken string          `toml:"unitypredict_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.dev.unitypredict.net",
	}
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(cliutil.EnvOverride("UNITYPREDICT_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := cliutil.EnvOverride("DEV_UNITYPREDICT_TOKEN"); v != "" {
		cfg.DevUnitypredictToken = v
		cfg.markEnvOverride("DevUnitypredictToken")
		cfg.AuthSource = "env:DEV_UNITYPREDICT_TOKEN"
		cfg.CredentialSource = "env:DEV_UNITYPREDICT_TOKEN"
	}
	...
	if v := cliutil.EnvOverride("UNITYPREDICT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("UNITYPREDICT_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "unitypredict-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DevUnitypredictToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DEV_UNITYPREDICT_TOKEN"
		}
		return applyAuthFormat("Bearer APIKEY@{token}", map[string]string{
			"unitypredict_token":     c.DevUnitypredictToken,
			"DEV_UNITYPREDICT_TOKEN": c.DevUnitypredictToken,
			"token":                  c.DevUnitypredictToken,
			"access_token":           c.DevUnitypredictToken,
		})
	}
	if c.AccessToken != "" {
		...
		return applyAuthFormat("Bearer APIKEY@{token}", map[string]string{"access_token": c.AccessToken, "token": c.AccessToken})
	}
	return ""
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.DevUnitypredictToken != "" {
		parts = append(parts, "unitypredict_token="+c.DevUnitypredictToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		DevUnitypredictToken: c.DevUnitypredictToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.DevUnitypredictToken == "" {
		c.DevUnitypredictToken = creds.DevUnitypredictToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.DevUnitypredictToken = ""
	delete(c.envOverrides, "DevUnitypredictToken")
	c.updateFileConfigField("DevUnitypredictToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["DevUnitypredictToken"] {
			out.DevUnitypredictToken = c.fileConfig.DevUnitypredictToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DevUnitypredictToken":
		c.fileConfig.DevUnitypredictToken = c.DevUnitypredictToken
	}
}
