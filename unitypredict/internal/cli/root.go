...
type rootFlags struct {
	...
	// invocation. Mirrors the UNITYPREDICT_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "unitypredict-pp-cli",
		Short: "The console-only parts of UnityPredict — model authoring, long descriptions, IO config, cross-env diffing — as CLI commands.",
		Long: `Manage unitypredict resources via the unitypredict API.
...
Run 'unitypredict-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("unitypredict-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'unitypredict-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAuthResourcePromotedCmd(flags))
	rootCmd.AddCommand(newFastPromotedCmd(flags))
	rootCmd.AddCommand(newPaymentsPromotedCmd(flags))
	rootCmd.AddCommand(newRepositoryPromotedCmd(flags))
	rootCmd.AddCommand(newSocialPromotedCmd(flags))
