...
The API's /api/models/search endpoint ignores text query params (measured
2026-09-19), so live text search does not exist for this API: auto and local
modes both use local FTS (run sync first); --data-source live explains and
exits. To filter models by name server-side, use
` + "`models list-usermodels --searchterm`" + `.`,
		Example: `  # Search (uses API endpoint if available, local FTS otherwise)
  unitypredict-pp-cli search "error timeout"

  # Force local search only
  unitypredict-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  unitypredict-pp-cli search "status" --type auth_resource --data-source local
  # JSON output for piping
  unitypredict-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API nominally has GET /api/models/search, but it ignores text
			// query params in every mode (measured 2026-09-19: totalCount and
			// ordering are identical for real and nonsense terms, keyword and
			// aisearch) — it cannot answer a text search. Text search is local
			// FTS (sync first) or models list-usermodels --searchterm.
			if flags.dataSource == "live" {
				return fmt.Errorf("live text search unavailable: GET /api/models/search ignores query params (measured); use --data-source local after `sync`, or `models list-usermodels --searchterm`")
			}
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'unitypredict-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
