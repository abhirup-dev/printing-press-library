...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unitypredict-pp-cli doctor
  unitypredict-pp-cli doctor --json
  unitypredict-pp-cli doctor --fail-on warn
  unitypredict-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | unitypredict-pp-cli auth set-token or export DEV_UNITYPREDICT_TOKEN=\"your-token-here\""
						report["auth_docs_url"] = "https://api.dev.unitypredict.net"
					} else {
						authConfigured = true
			...
			if os.Getenv("DEV_UNITYPREDICT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "DEV_UNITYPREDICT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DEV_UNITYPREDICT_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "unitypredict-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'unitypredict-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'unitypredict-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'unitypredict-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'unitypredict-pp-cli sync' to refresh."
	}
	return report
}
