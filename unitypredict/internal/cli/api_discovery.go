...
Run 'api <interface>' to see that interface's methods.`,
  ...
  unitypredict-pp-cli api

  # Show methods for a specific interface
  unitypredict-pp-cli api <interface-name>`,
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Annotations["pp:api-resource"] == "true" && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-pp-cli %s <method> --help' for details.\n", "unitypredict", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-pp-cli api' to list all interfaces", args[0], "unitypredict")
			}
