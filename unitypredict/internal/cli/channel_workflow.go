...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  unitypredict-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  unitypredict-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  unitypredict-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"auth_resource", "engines", "models"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  unitypredict-pp-cli workflow status

  # Show status as JSON
  unitypredict-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("unitypredict-pp-cli")
			}
