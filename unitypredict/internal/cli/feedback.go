...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNITYPREDICT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNITYPREDICT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "unitypredict-pp-cli/feedback")
...
When ` + "`UNITYPREDICT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNITYPREDICT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  unitypredict-pp-cli feedback "docs said exclusive but the boundary is inclusive"
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unitypredict-pp-cli feedback list
  unitypredict-pp-cli feedback list --limit 5
  unitypredict-pp-cli feedback list --json`,
