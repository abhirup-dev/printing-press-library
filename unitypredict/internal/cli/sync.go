...
Resource scoping:
  ...
  unitypredict-pp-cli sync
  # Sync specific resources only
  unitypredict-pp-cli sync --resources auth_resource,clients

  # Full resync (ignore previous checkpoint)
  unitypredict-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  unitypredict-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  unitypredict-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  unitypredict-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "engines":
		return db.UpsertEngines(data)
	case "models":
		return db.UpsertModels(data)
	case "repository":
		return db.UpsertRepository(data)
	case "tools":
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"payments",
		"tools",
		"tools-public-dev-abhirupdas-8-c0-af724114435",
	}
	return names
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"auth_resource":            "/api/auth",
		"clients":                  "/api/clients/Console",
		"clients-router":           "/api/clients/router/",
		"engines":                  "/api/engines/userengines",
		"engines-supportedengines": "/api/engines/supportedengines",
		"models":                   "/api/models/usermodels",
		"payments":                 "/api/payments/accounts",
		"tools":                    "/api/tools/analytics",
		"tools-public-dev-abhirupdas-8-c0-af724114435": "/api/tools/public/dev.abhirupdas_8C0AF724114435",
	}
