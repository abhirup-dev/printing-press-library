...
var whichIndex = []whichEntry{
	{Command: "logs get", Description: "One command fetches the per-request devLog, the runtime Log.txt, and the engine build log, in order.", Group: "Diagnostics", WhyItMatters: "When a predict fails you want every log granularity at once, not three manual URL dances that break if you send the auth header to S3."},
	{Command: "models describe set", Description: "Publish a model's long description from a Markdown file in one step, including the S3 upload dance.", Group: "Model authoring", WhyItMatters: "The long description is a hosted file, not a text field — writing prose into the upsert body is silently ignored, so automation needs the exact 3-step chain."},
	{Command: "models io set", Description: "Define a model's input and outcome variables from a YAML file instead of the console's per-field editors.", Group: "Model authoring", WhyItMatters: "The GET-mutate-POST full-record discipline and the hidden-variable display map are only automatable if one command owns the whole round-trip."},
	{Command: "engines models", Description: "List every model built on a given engine.", Group: "Engine lifecycle", WhyItMatters: "Before redeploying or deleting an engine you must know its blast radius; the console buries this."},
	{Command: "auth check", Description: "Validate credentials against the one endpoint that cannot lie about auth.", Group: "Auth and safety", WhyItMatters: "Prevents shipping on a wrong-env or malformed key that appeared to work because a list route answered 200."},
	{Command: "engines concurrency", Description: "Show how many engine nodes ran in parallel over time, as a table.", Group: "Engine lifecycle", WhyItMatters: "Answers capacity questions (did we actually run 4 in parallel?) without the console."},
	{Command: "predict run", Description: "Invoke a model with text or file inputs, poll to completion, and pull File outcomes into a directory.", Group: "Invocation", WhyItMatters: "Cold-started GPU models take minutes; a single wait-and-fetch command is the difference between a script and a one-liner."},
	{Command: "diff model", Description: "Compare a model or engine record across dev and prod, field by field.", Group: "Promotion", WhyItMatters: "Before promoting, you want to know exactly which fields drifted between dev and prod."},
	{Command: "auth-resource", Description: "GET /api/auth", Group: "auth-resource", WhyItMatters: "GET /api/auth"}, // pp:which-promoted
	{Command: "fast", Description: "GET /api/fast/models/{model_id}/files/THUMBNAIL.png", Group: "fast", WhyItMatters: "GET /api/fast/models/{model_id}/files/THUMBNAIL."}, // pp:which-promoted
	{Command: "payments", Description: "GET /api/payments/accounts", Group: "payments", WhyItMatters: "GET /api/payments/accounts"}, // pp:which-promoted
	{Command: "repository", Description: "GET /api/repository/search", Group: "repository", WhyItMatters: "GET /api/repository/search"}, // pp:which-promoted
	{Command: "social", Description: "GET /api/social/model/review/{review_id}", Group: "social", WhyItMatters: "GET /api/social/model/review/{review_id}"}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  unitypredict-pp-cli which "stale tickets"
  unitypredict-pp-cli which "bottleneck"
  unitypredict-pp-cli which --limit 1 "send message"
  unitypredict-pp-cli which                                # list the full capability index`,
