...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Unitypredict",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  unitypredict-pp-cli auth setup\n  unitypredict-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://api.dev.unitypredict.net")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export DEV_UNITYPREDICT_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  echo \"$TOKEN\" | unitypredict-pp-cli auth set-token")
			if !launch {
				return nil
			}
			launchURL := "https://api.dev.unitypredict.net"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  unitypredict-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export DEV_UNITYPREDICT_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  echo \"$TOKEN\" | unitypredict-pp-cli auth set-token\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | unitypredict-pp-cli auth set-token\n  unitypredict-pp-cli auth set-token < token-file",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  unitypredict-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("DEV_UNITYPREDICT_TOKEN") != "" {
				envStillSet = "DEV_UNITYPREDICT_TOKEN"
			}
