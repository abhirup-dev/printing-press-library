...
func newReportsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "reports",
		Short:       "Reports, results, concalls, and upcoming events",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newReportsConcallsCmd(flags))
	cmd.AddCommand(newReportsListCmd(flags))
	cmd.AddCommand(newReportsQuarterlyResultsCmd(flags))
	cmd.AddCommand(newReportsUpcomingCmd(flags))
	return cmd
}
