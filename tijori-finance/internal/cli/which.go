...
var whichIndex = []whichEntry{
	{Command: "research peers", Description: "Compare live peer financial, ownership, and operating evidence in one unit-aware matrix.", Group: "Evidence-linked research", WhyItMatters: "Use it when a screener hit needs mechanically comparable evidence across peers."},
	{Command: "research review-queue", Description: "Join current holdings or watchlist names with upcoming results, events, reports, and alerts.", Group: "Evidence-linked research", WhyItMatters: "Use it for a focused review ritual instead of opening every company page manually."},
	{Command: "research changes", Description: "Emit deterministic row-level changes between two server-returned financial periods.", Group: "Evidence-linked research", WhyItMatters: "Use it when the question is what changed, not just what the latest table says."},
	{Command: "research ownership-check", Description: "Align same-period ownership movements with financial and cash-flow changes.", Group: "Evidence-linked research", WhyItMatters: "Use it to inspect ownership changes alongside fundamentals without inferring investment advice."},
	{Command: "research result-pack", Description: "Connect a result or event to report, concall, and validated source-document metadata.", Group: "Evidence-linked research", WhyItMatters: "Use it when a number needs its filing or investor document attached."},
	{Command: "screener run", Description: "Return the exact native query, matched fields, result page, and request provenance (use --explain).", Group: "Agent-native plumbing", WhyItMatters: "Use it when a screen must be inspected or handed to another agent exactly as submitted."},
	{Command: "research context", Description: "Bundle dated company, sector, macro, and raw-material observations without a synthetic verdict.", Group: "Cross-surface context", WhyItMatters: "Use it when company performance needs its operating environment beside it."},
	{Command: "portfolio quality", Description: "Join live portfolio weights with public company quality evidence and explicit unavailable states.", Group: "Cross-surface context", WhyItMatters: "Use it for a current exposure review that does not silently drop inaccessible names."},
	{Command: "feed inspect", Description: "Resolve timeline items to normalized event, report, and document metadata while preserving cursors.", Group: "Evidence-linked research", WhyItMatters: "Use it when an event feed item needs the underlying source and continuation state."},
	{Command: "auth capabilities", Description: "Report public, authenticated, empty, blocked, expired, or changed-shape status per safe GET endpoint.", Group: "Reachability mitigation", WhyItMatters: "Use it before a portfolio-aware workflow when session reachability is uncertain."},
	{Command: "research input-context", Description: "Place current raw-material observations beside live operational and revenue-mix data.", Group: "Cross-surface context", WhyItMatters: "Use it when input prices are part of the research question but a buy/sell conclusion is not."},
	{Command: "alerts", Description: "List active or historical alerts", Group: "alerts", WhyItMatters: "List active or historical alerts"}, // pp:which-promoted
	{Command: "ideas", Description: "Fetch ideas dashboard metadata and links", Group: "ideas", WhyItMatters: "Fetch ideas dashboard metadata and links"}, // pp:which-promoted
	{Command: "macro", Description: "Fetch industry, demand, and GDP tables", Group: "macro", WhyItMatters: "Fetch industry, demand, and GDP tables"}, // pp:which-promoted
	{Command: "raw-materials", Description: "Fetch chemical, spread, and metal price tables", Group: "raw-materials", WhyItMatters: "Fetch chemical, spread, and metal price tables"}, // pp:which-promoted
	{Command: "watchlist", Description: "Fetch one watchlist data page", Group: "watchlist", WhyItMatters: "Fetch one watchlist data page"}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  tijori-finance-pp-cli which "stale tickets"
  tijori-finance-pp-cli which "bottleneck"
  tijori-finance-pp-cli which --limit 1 "send message"
  tijori-finance-pp-cli which                                # list the full capability index`,
