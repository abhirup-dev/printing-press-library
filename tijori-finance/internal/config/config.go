...
func Load(configPath string) (*Config, error) {
	...
	if v := cliutil.EnvOverride("TIJORI_FINANCE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	// A browser-established session can be passed for one process without
	// writing its credential to disk. This is intentionally read only at load
	// time; callers must not echo the value or persist it in shell history.
	if v := os.Getenv("TIJORI_FINANCE_AUTH_HEADER"); v != "" {
		cfg.AuthHeaderVal = v
		cfg.AuthSource = "environment"
	} else {
		if material, source, storeErr := ReadStoredAuthMaterial(); storeErr == nil {
			if cfg.AuthHeaderVal == "" {
				cfg.AuthHeaderVal = material.AuthHeader
			}
			...
			if cfg.AuthSource == "" && (material.AuthHeader != "" || len(material.Headers) > 0) {
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("TIJORI_FINANCE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "tijori-finance-pp-cli", "config.toml"), nil
}
