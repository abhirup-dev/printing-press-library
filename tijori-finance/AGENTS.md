# Tijori Finance Printed CLI Agent Guide

This directory is a generated `tijori-finance-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
tijori-finance-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tijori-finance-pp-cli which "<capability>" --json
tijori-finance-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
tijori-finance-pp-cli <command> --agent
```
...
```bash
tijori-finance-pp-cli <command> --help
tijori-finance-pp-cli <command> --dry-run --agent
```
...
Every hand-written novel command is live-only. Keep `cmd.Annotations["pp:data-source"]` set to `live`; `--agent` envelopes read that annotation for `meta.source`. Reject local/auto data-source requests with a clear error. Do not add TODO stubs, caches, sync, history, snapshots, or mutation paths.
