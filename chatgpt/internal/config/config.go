...
type Config struct {
	...
	ChatgptToken     string          `json:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(cliutil.EnvOverride("CHATGPT_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}
	if v := cliutil.EnvOverride("CHATGPT_TOKEN"); v != "" {
		cfg.ChatgptToken = v
		cfg.markEnvOverride("ChatgptToken")
		cfg.AuthSource = "env:CHATGPT_TOKEN"
		cfg.CredentialSource = "env:CHATGPT_TOKEN"
	}
	...
	if v := cliutil.EnvOverride("CHATGPT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CHATGPT_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "chatgpt-pp-cli", "config.json"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ChatgptToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CHATGPT_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":         c.ChatgptToken,
			"CHATGPT_TOKEN": c.ChatgptToken,
			"access_token":  c.ChatgptToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.ChatgptToken != "" {
		parts = append(parts, "token="+c.ChatgptToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ChatgptToken == "" {
		c.ChatgptToken = creds.ChatgptToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ChatgptToken = ""
	delete(c.envOverrides, "ChatgptToken")
	c.updateFileConfigField("ChatgptToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["ChatgptToken"] {
			out.ChatgptToken = c.fileConfig.ChatgptToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ChatgptToken":
		c.fileConfig.ChatgptToken = c.ChatgptToken
	}
}
