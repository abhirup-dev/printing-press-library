...
type rootFlags struct {
	...
	// invocation. Mirrors the CHATGPT_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "chatgpt-pp-cli",
		Short: `Your ChatGPT history as a queryable local archive — verified-complete extraction, outlines, and search no other tool has.`,
		Long: `Your ChatGPT history as a queryable local archive — verified-complete extraction, outlines, and search no other tool has.
...
Health check: run 'chatgpt-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("chatgpt-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select items,total,limit)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'chatgpt-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// the command proceeds with the stale cache either way. --help
		// must not open the store or migrate as a side effect of help.
	...
	rootCmd.AddCommand(newGlobalSearchPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelListCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelOutlineCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTranscriptCmd(flags))
