...
var whichIndex = []whichEntry{
	{Command: "list", Description: "See every chat in chronological order with turn counts, time span, and freshness — not just titles.", Group: "Inventory that answers questions", WhyItMatters: "Pick this when you need a real index of conversations by time and size instead of scrolling the sidebar."},
	{Command: "transcript", Description: "Extract an entire conversation with a completeness proof against the live tree state.", Group: "Inventory that answers questions", WhyItMatters: "Use this when an export must contain every message — audit, migration, or feeding another agent."},
	{Command: "outline", Description: "Map a long thread into titled sections at each of your prompts, with timestamps and anchors.", Group: "Long-thread navigation", WhyItMatters: "Use this before reading a 100-message thread — get the structure first, then pull only the section you need."},
	{Command: "transcript", Description: "Extract just part of a long conversation by section number or message anchors.", Group: "Long-thread navigation", WhyItMatters: "Use this to pull one topic out of a sprawling thread without extracting everything."},
	{Command: "find", Description: "Search message content server-side by default, or offline over your synced archive.", Group: "Inventory that answers questions", WhyItMatters: "Pick this to find the exact conversation and message containing a phrase, fresh or from cache."},
	{Command: "global-search", Description: "Federated message-content search across conversations and projects — strict body schema (extra fields 422)", Group: "global-search", WhyItMatters: "Federated message-content search across conversations and projects — strict body schema (extra fields 422)"}, // pp:which-promoted
	{Command: "models", Description: "Model picker inventory — slugs, reasoning types, thinking efforts, default badge", Group: "models", WhyItMatters: "Model picker inventory — slugs, reasoning types, thinking efforts, default badge"}, // pp:which-promoted
	{Command: "session", Description: "Current session, account, and freshly minted access token (values are credential material — never logged)", Group: "session", WhyItMatters: "Current session, account, and freshly minted access token (values are credential material — never logged)"}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  chatgpt-pp-cli which "stale tickets"
  chatgpt-pp-cli which "bottleneck"
  chatgpt-pp-cli which --limit 1 "send message"
  chatgpt-pp-cli which                                # list the full capability index`,
