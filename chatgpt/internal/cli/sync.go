...
Resource scoping:
  ...
  chatgpt-pp-cli sync --resources conversations,models

  # Full resync (ignore previous checkpoint)
  chatgpt-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  chatgpt-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  chatgpt-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  chatgpt-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "chatgpt-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
