...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CHATGPT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CHATGPT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "chatgpt-pp-cli/feedback")
...
When ` + "`CHATGPT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CHATGPT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  chatgpt-pp-cli feedback "docs said exclusive but the boundary is inclusive"
  chatgpt-pp-cli feedback --stdin < notes.txt
  chatgpt-pp-cli feedback list`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  chatgpt-pp-cli feedback list
  chatgpt-pp-cli feedback list --limit 5
  chatgpt-pp-cli feedback list --json`,
