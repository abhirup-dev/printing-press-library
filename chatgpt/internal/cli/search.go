...
In local mode: searches locally synced data only.`,
  ...
  chatgpt-pp-cli search "error timeout"

  # Force local search only
  chatgpt-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  chatgpt-pp-cli search "status" --type chat --data-source local
  # JSON output for piping
  chatgpt-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /backend-api/global/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/backend-api/global/search", map[string]any{
					"query":  query,
					"limit":  10,
					"cursor": "",
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'chatgpt-pp-cli sync --resources conversations,models' first to populate the local database.", err)
			}
			defer db.Close()
