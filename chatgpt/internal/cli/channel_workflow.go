...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  chatgpt-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  chatgpt-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  chatgpt-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  chatgpt-pp-cli workflow status

  # Show status as JSON
  chatgpt-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("chatgpt-pp-cli")
			}
