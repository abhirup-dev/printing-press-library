...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  chatgpt-pp-cli doctor
  chatgpt-pp-cli doctor --json
  chatgpt-pp-cli doctor --fail-on warn
  chatgpt-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | chatgpt-pp-cli auth set-token or export CHATGPT_TOKEN=\"your-token-here\""
						report["auth_docs_url"] = "https://chatgpt.com"
					} else {
						authConfigured = true
			...
			if os.Getenv("CHATGPT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CHATGPT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CHATGPT_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "chatgpt-pp-cli", suggestion)
						} else {
			...
			report["cache"] = collectCacheReport(cmd.Context(), "168h")
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'chatgpt-pp-cli sync --resources conversations,models' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'chatgpt-pp-cli sync --resources conversations,models' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'chatgpt-pp-cli sync --resources conversations,models' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'chatgpt-pp-cli sync --resources conversations,models' to refresh."
	}
	return report
}
