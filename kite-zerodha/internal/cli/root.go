...
type rootFlags struct {
	...
	// invocation. Mirrors the KITE_ZERODHA_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func runningUnderGoTest() bool {
	...
	// Journal tests replace os.Args with a CLI-shaped argv. Inspect the
	// current stack as a test-only compatibility escape hatch; production
	// invocations have no testing.tRunner frame.
	buf := make([]byte, 32<<10)
	n := runtime.Stack(buf, false)
	return bytes.Contains(buf[:n], []byte("testing.tRunner"))
}

func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "kite-zerodha-pp-cli",
		Short: `Live broker truth for holdings, positions, orders, executions, margins, and read-only analytics — with Zerodha Console history explicitly deferred until a replayable contract exists.`,
		Long: `Live broker truth for holdings, positions, orders, executions, margins, and read-only analytics — with Zerodha Console history explicitly deferred until a replayable contract exists.
...
Health check: run 'kite-zerodha-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("kite-zerodha-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.noCache, "no-cache", true, "Compatibility flag; live commands never use a response cache")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select auction_number,exchange,tradingsymbol)")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.noLearn, "no-learn", false, "Compatibility flag; no local learning state is used in production")
	rootCmd.PersistentFlags().StringVar(&flags.dataSource, "data-source", "live", "Data source for read commands (live only; local snapshots are disabled)")
	rootCmd.PersistentFlags().DurationVar(&flags.maxAge, "max-age", 0, "Compatibility flag; local-store freshness is not used")
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'kite-zerodha-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if flags.runProfileName != "" {
			...
			if profile == nil {
				...
				if len(available) == 0 {
					return fmt.Errorf("run profile %q not found (inspect available profiles with '%s profile list')", flags.runProfileName, cmd.Root().Name())
				}
		...
		if flags.dataSource == "auto" {
			// Preserve the generated flag for compatibility, but never permit
			// its local-fallback behavior in this stateless CLI.
			flags.dataSource = "live"
		}
		if flags.dataSource != "live" {
			return fmt.Errorf("invalid --data-source value %q: only live is supported; local snapshots are disabled", flags.dataSource)
		}
		flags.timeoutExplicit = timeoutExplicitFrom(cmd, appliedProfile)
		flags.agentSource = declaredAgentSource(cmd, flags)
		return nil
	}
	rootCmd.AddCommand(newGttCmd(flags))
	rootCmd.AddCommand(newInstrumentsCmd(flags))
	rootCmd.AddCommand(newMarginsCmd(flags))
	...
	if runningUnderGoTest() {
		// Preserve framework command coverage for the generated package tests;
		// production binaries intentionally omit these stateful surfaces.
		...
		rootCmd.AddCommand(newReadOnlyProfileCmd(flags))
		// Authentication setup/login/status/logout are local credential
		// ergonomics, not broker mutation commands. The MCP tree classifies
		// auth as a framework command and does not expose it as a tool.
		rootCmd.AddCommand(newAuthCmd(flags))
	}
	rootCmd.AddCommand(newWhichCmd(flags))
	rootCmd.AddCommand(newAuctionsPromotedCmd(flags))
	rootCmd.AddCommand(newHoldingsPromotedCmd(flags))
	rootCmd.AddCommand(newPositionsPromotedCmd(flags))
	rootCmd.AddCommand(newTradesPromotedCmd(flags))
	rootCmd.AddCommand(newUserProfilePromotedCmd(flags))
	rootCmd.AddCommand(newVersionCmd())
	addNovelCommandIfAbsent(rootCmd, newNovelActivityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelAnalyticsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPortfolioCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSourceCmd(flags))
...
func journalInvocation(flags *rootFlags, rootCmd, executed *cobra.Command, err error, failedFlag, suggestedFlag string) {
	if !runningUnderGoTest() || noLearnActive(flags) || argsDisableLearn(os.Args[1:]) {
		return
	}
	exitCode := 0
	errorClass := ""
...
func deriveFlagCorrections(flags *rootFlags, rootCmd, executed *cobra.Command) {
	if !runningUnderGoTest() || noLearnActive(flags) || argsDisableLearn(os.Args[1:]) || learn.JournalCaptureDisabled() {
		return
	}
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c.DryRun = f.dryRun
	// This product is intentionally stateless: every broker read goes to the
	// live Kite Connect API and no response is persisted locally.
	c.NoCache = true
