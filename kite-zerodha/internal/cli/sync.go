...
Resource scoping:
  ...
  kite-zerodha-pp-cli sync --resources auctions,gtt

  # Full resync (ignore previous checkpoint)
  kite-zerodha-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  kite-zerodha-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  kite-zerodha-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  kite-zerodha-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "auctions":
		return db.UpsertAuctions(data)
	case "gtt":
		return db.UpsertGtt(data)
	case "holdings":
		return db.UpsertHoldings(data)
	case "margins":
		return db.UpsertMargins(data)
	case "orders":
		return db.UpsertOrders(data)
	case "trades":
		return db.UpsertTrades(data)
	case "user_profile":
		return db.UpsertUserProfile(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"auctions":    "/portfolio/holdings/auctions",
		"gtt":         "/gtt/triggers",
