...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  kite-zerodha-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  kite-zerodha-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  kite-zerodha-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"gtt", "instruments", "orders", "trades"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  kite-zerodha-pp-cli workflow status

  # Show status as JSON
  kite-zerodha-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("kite-zerodha-pp-cli")
			}
