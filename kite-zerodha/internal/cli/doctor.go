...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kite-zerodha-pp-cli doctor
  kite-zerodha-pp-cli doctor --json
  kite-zerodha-pp-cli doctor --fail-on warn
  kite-zerodha-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | kite-zerodha-pp-cli auth set-token or export KITE_CONNECT_TOKEN=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("KITE_CONNECT_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "KITE_CONNECT_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "KITE_CONNECT_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "kite-zerodha-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("kite-zerodha-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'kite-zerodha-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'kite-zerodha-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'kite-zerodha-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'kite-zerodha-pp-cli sync' to refresh."
	}
	return report
}
