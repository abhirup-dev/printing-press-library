...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KITE_ZERODHA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KITE_ZERODHA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kite-zerodha-pp-cli/feedback")
...
When ` + "`KITE_ZERODHA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KITE_ZERODHA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  kite-zerodha-pp-cli feedback "docs said exclusive but the boundary is inclusive"
  kite-zerodha-pp-cli feedback --stdin < notes.txt
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kite-zerodha-pp-cli feedback list
  kite-zerodha-pp-cli feedback list --limit 5
  kite-zerodha-pp-cli feedback list --json`,
