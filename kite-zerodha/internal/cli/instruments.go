...
func newInstrumentsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "instruments",
		Short:       "Daily Kite instrument master",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newInstrumentsExchangeCmd(flags))
	cmd.AddCommand(newInstrumentsListCmd(flags))
	return cmd
}
