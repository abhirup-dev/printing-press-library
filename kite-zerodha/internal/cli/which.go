...
var whichIndex = []whichEntry{
	{Command: "portfolio summary", Description: "Combine live holdings, positions, and margin state into one current portfolio view.", Group: "Live broker analytics", WhyItMatters: "Use when an agent needs the current broker-grounded portfolio in one response."},
	{Command: "activity buys", Description: "Summarize current-day buy executions with quantities, prices, and turnover.", Group: "Live broker analytics", WhyItMatters: "Use for a current-day buy or sell summary without mistaking orders for fills."},
	{Command: "activity fills", Description: "Show each order with the executions and partial fills it spawned.", Group: "Execution reasoning", WhyItMatters: "Use when explaining how a specific order actually executed."},
	{Command: "analytics overview", Description: "Compute current P&L, concentration, turnover, and win/loss summaries from live rows.", Group: "Live broker analytics", WhyItMatters: "Use for a current-state portfolio analysis when a local history store is prohibited."},
	{Command: "source status", Description: "Report which broker and deferred Console capabilities are safe to use.", Group: "Transport boundaries", WhyItMatters: "Use before asking for historical tradebook, True P&L, charges, or corporate-action-aware history."},
	{Command: "auctions", Description: "List holdings auctions", Group: "auctions", WhyItMatters: "List holdings auctions"}, // pp:which-promoted
	{Command: "holdings", Description: "List current long-term holdings", Group: "holdings", WhyItMatters: "List current long-term holdings"}, // pp:which-promoted
	{Command: "positions", Description: "Get current net and day positions", Group: "positions", WhyItMatters: "Get current net and day positions"}, // pp:which-promoted
	{Command: "trades", Description: "List current-day trades and executions", Group: "trades", WhyItMatters: "List current-day trades and executions"}, // pp:which-promoted
	{Command: "user-profile", Description: "Get the authenticated Kite profile", Group: "user-profile", WhyItMatters: "Get the authenticated Kite profile"}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  kite-zerodha-pp-cli which "stale tickets"
  kite-zerodha-pp-cli which "bottleneck"
  kite-zerodha-pp-cli which --limit 1 "send message"
  kite-zerodha-pp-cli which                                # list the full capability index`,
