...
Run sync first to populate the local search index.`,
  ...
  kite-zerodha-pp-cli search "status"

  # Force local search explicitly
  kite-zerodha-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  kite-zerodha-pp-cli search "status" --type auctions --data-source local
  # JSON output for piping
  kite-zerodha-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'kite-zerodha-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
