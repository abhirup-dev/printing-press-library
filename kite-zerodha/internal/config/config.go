...
type Config struct {
	...
	KiteConnectToken string          `toml:"connect_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := cliutil.EnvOverride("KITE_CONNECT_TOKEN"); v != "" {
		cfg.KiteConnectToken = v
		cfg.markEnvOverride("KiteConnectToken")
		cfg.AuthSource = "env:KITE_CONNECT_TOKEN"
		cfg.CredentialSource = "env:KITE_CONNECT_TOKEN"
	}
	...
	if v := cliutil.EnvOverride("KITE_ZERODHA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("KITE_ZERODHA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "kite-zerodha-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.KiteConnectToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KITE_CONNECT_TOKEN"
		}
		return applyAuthFormat("token {token}", map[string]string{
			"connect_token":      c.KiteConnectToken,
			"KITE_CONNECT_TOKEN": c.KiteConnectToken,
			"token":              c.KiteConnectToken,
			"access_token":       c.KiteConnectToken,
		})
	}
	if c.AccessToken != "" {
		...
		return applyAuthFormat("token {token}", map[string]string{"access_token": c.AccessToken, "token": c.AccessToken})
	}
	return ""
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.KiteConnectToken != "" {
		parts = append(parts, "connect_token="+c.KiteConnectToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		KiteConnectToken: c.KiteConnectToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.KiteConnectToken == "" {
		c.KiteConnectToken = creds.KiteConnectToken
	}
}
...
// SaveKiteConnectToken stores the composite api_key:access_token credential
// in the approved credentials store. The API secret is deliberately not
// persisted: it is only used for the request-token exchange.
func (c *Config) SaveKiteConnectToken(apiKey, accessToken string) error {
	apiKey = strings.TrimSpace(apiKey)
	accessToken = strings.TrimSpace(accessToken)
	if apiKey == "" || accessToken == "" {
		return fmt.Errorf("api key and access token are required")
	}
	c.AuthHeaderVal = ""
	c.KiteConnectToken = apiKey + ":" + accessToken
	c.AccessToken = ""
	c.RefreshToken = ""
	c.TokenExpiry = time.Time{}
	// Keep the API key/secret in the private credentials store so the next
	// login can reuse them without putting the secret in process arguments.
	c.ClientID = apiKey
	c.updateFileConfigField("AuthHeaderVal")
	c.updateFileConfigField("KiteConnectToken")
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.KiteConnectToken = ""
	delete(c.envOverrides, "KiteConnectToken")
	c.updateFileConfigField("KiteConnectToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KiteConnectToken"] {
			out.KiteConnectToken = c.fileConfig.KiteConnectToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KiteConnectToken":
		c.fileConfig.KiteConnectToken = c.KiteConnectToken
	}
}
