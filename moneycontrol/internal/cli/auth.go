...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Moneycontrol",
...
func requiredAuthCookies() []string {
	raw := []string{"mc-auth-log", "nousersess", "stocks"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for moneycontrol.com.
...
profile by name when the installed backend supports it.`,
		Example: `  moneycontrol-pp-cli auth login --chrome
  moneycontrol-pp-cli auth login --browser
  moneycontrol-pp-cli auth login --cookies-file storage-state.json
  moneycontrol-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  moneycontrol-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  moneycontrol-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  moneycontrol-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
			...
			if !fromPressAuth && !fromCookiesFile {
				...
				if err != nil {
					...
					if runtime.GOOS == "windows" {
						...
						fmt.Fprintf(w, "\n  moneycontrol-pp-cli auth login --cookies-file <path>\n\n")
			...
			requiredCookies := []string{"mc-auth-log", "nousersess", "stocks"}
			// Moneycontrol's account endpoints sit behind Akamai. The three
			// account cookies establish identity; these optional WAF cookies
			// are replayed only when the browser supplied them, otherwise a
			// valid authenticated session can look like an anonymous 503.
			persistCookies := append(append([]string{}, requiredCookies...), "_abck", "bm_sz", "bm_so", "bm_s", "bm_sv")
			kept := make([]string, 0, len(persistCookies))
			...
			for _, name := range persistCookies[len(requiredCookies):] {
				if v, ok := cookieMap[name]; ok && strings.TrimSpace(v) != "" {
					kept = append(kept, name+"="+v)
				}
			}
			cookies = strings.Join(kept, "; ")

			// Step 5: Save browser material in the dedicated secure store. The
			// raw Cookie header never enters config.json, credentials.toml, argv,
			// stdout, or a receipt.
			backend, err := config.StoreAuthMaterial(config.SessionMaterial{Cookie: cookies, Domain: domain})
			if err != nil {
				return configErr(fmt.Errorf("saving cookies: %w", err))
			}
			// Persist the selected cookies so response rotation survives the
			// next invocation; the secure store remains the source of truth.
			jarCookies := make(map[string]string, len(persistCookies))
			for _, name := range persistCookies {
				if v, ok := cookieMap[name]; ok && strings.TrimSpace(v) != "" {
					jarCookies[name] = v
				}
			}
			...
			fmt.Fprintf(w, "Session saved to secure backend %s\n", backend)
			return nil
		},
	}
...
func newAuthImportCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "import",
		Short:       "Read browser session material from stdin and store it securely",
		Example:     `  ego-browser-session | moneycontrol-pp-cli auth import --stdin --json`,
		Annotations: map[string]string{"mcp:read-only": "false", "pp:data-source": "computed", "pp:happy-args": "--stdin"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if !fromStdin {
				return authErr(fmt.Errorf("auth import requires --stdin; pipe browser session material without printing it"))
			}
			data, err := io.ReadAll(io.LimitReader(cmd.InOrStdin(), 64*1024))
			if err != nil {
				return authErr(fmt.Errorf("read auth material from stdin: %w", err))
			}
			value := strings.TrimSpace(string(data))
			if value == "" {
				return authErr(fmt.Errorf("stdin must contain one session-material JSON object"))
			}
			var payload struct {
			...
			if err := json.Unmarshal([]byte(value), &payload); err != nil {
				return authErr(fmt.Errorf("stdin session material is not valid JSON"))
			}
			cookieMap := parseCookieString(payload.Cookie)
			for _, name := range requiredAuthCookies() {
				if strings.TrimSpace(cookieMap[name]) == "" {
					return authErr(fmt.Errorf("stdin session material is missing required cookie %q", name))
				}
			}
			if payload.Domain == "" {
				payload.Domain = "moneycontrol.com"
			}
			backend, err := config.StoreAuthMaterial(config.SessionMaterial{Cookie: payload.Cookie, Headers: payload.Headers, Domain: payload.Domain})
			if err != nil {
				return authErr(err)
			}
			return flags.printJSON(cmd, map[string]any{"stored": true, "backend": backend, "credential_values_included": false, "cookie_names": len(cookieMap)})
		},
	}
	cmd.Flags().BoolVar(&fromStdin, "stdin", false, "Read session material from stdin")
	return cmd
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "status",
		Short:   "Show sanitized auth-store state and a live identity probe",
		Example: "  moneycontrol-pp-cli auth status --json",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			probe := map[string]any{"state": "not_configured", "endpoint": "/user/get_userid_details/"}
			if credentialConfigured {
				probe = probeAuthIdentity(cmd, cfg, flags)
			}
			state := "authenticated"
			if !credentialConfigured {
				state = "not_configured"
			} else if s, ok := probe["state"].(string); ok && s != "ok" {
				state = s
			}
			if flags.asJSON {
				return flags.printJSON(cmd, map[string]any{
					...
					"credential_source":     cfg.AuthSource,
					"credential_domain":     "moneycontrol.com",
					"store":                 config.AuthStoreMetadata(),
					"live_probe":            probe,
				})
			}
			if !credentialConfigured {
				...
				fmt.Fprintf(w, "  moneycontrol-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
			if state == "ok" {
				fmt.Fprintln(w, green("Authenticated"))
			} else {
				fmt.Fprintf(w, "%s\n", red("Authenticated session probe: "+state))
			}
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: moneycontrol.com\n")
			fmt.Fprintf(w, "  Live probe: %s\n", state)
...
func probeAuthIdentity(cmd *cobra.Command, cfg *config.Config, flags *rootFlags) map[string]any {
	result := map[string]any{"state": "upstream_error", "endpoint": "/user/get_userid_details/"}
	c := client.New(cfg, flags.timeout, flags.rateLimit)
	c.NoCache = true
	status, err := c.ProbeGet(cmd.Context(), "/user/get_userid_details/")
	...
	result["status_code"] = status
	result["observed_at"] = time.Now().UTC().Format(time.RFC3339)
	result["stability"] = "fragile"
	result["remediation"] = "Retry later; do not treat this response as unauthenticated."
	if apiErr, ok := err.(*client.APIError); ok {
		switch {
		case apiErr.StatusCode == 401 || apiErr.StatusCode == 403:
			result["state"] = "entitlement_denied"
		case apiErr.StatusCode >= 500:
			result["state"] = "upstream_error"
		case apiErr.StatusCode == 429:
			result["state"] = "blocked"
		default:
			result["state"] = "shape_changed"
		}
	} else if err != nil {
		result["state"] = "upstream_error"
	}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | moneycontrol-pp-cli auth set-token\n  moneycontrol-pp-cli auth set-token < token-file",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  moneycontrol-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if _, err := config.ClearStoredAuthMaterial(); err != nil {
				return configErr(fmt.Errorf("clearing secure auth store: %w", err))
			}
