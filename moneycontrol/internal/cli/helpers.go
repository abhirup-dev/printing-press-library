...
func classifyHTMLTransportError(err error) error {
	...
	if htmlLooksLikeAuthFailure(evidence) {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'moneycontrol-pp-cli auth login --chrome' to refresh browser-session credentials."))
	}
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'moneycontrol-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			" Run 'moneycontrol-pp-cli auth login --chrome' to refresh browser-session credentials."+
			"\n      See API docs: https://www.moneycontrol.com"+
			"\n      Run 'moneycontrol-pp-cli doctor' to check auth status.", err))
...
func classifyAPIError(w io.Writer, err error, flags *rootFlags) error {
	...
	if detail, endpoint, ok := knownFragileUpstreamError(err); ok {
		writeKnownFragileEnvelope(w, flags, endpoint, detail)
		return nil
	}
...
// classifyAccountReadError keeps browser-confirmed Moneycontrol account-route
// failures distinct from authentication failures. These routes are fragile
// Akamai/WAF surfaces: both the authenticated browser and CLI can receive an
// empty 503 response. Never turn that observation into "unauthenticated" or an
// empty successful result.
func classifyAccountReadError(w io.Writer, err error, flags *rootFlags, endpoint string) error {
	if err == nil {
		return nil
	}
	detail := "upstream response unavailable"
	isUpstream := false
	var apiErrValue *client.APIError
	if errors.As(err, &apiErrValue) && apiErrValue.StatusCode >= 500 {
		isUpstream = true
		detail = fmt.Sprintf("HTTP %d", apiErrValue.StatusCode)
	} else if strings.Contains(strings.ToLower(err.Error()), "empty response body") {
		isUpstream = true
	...
	message := "Moneycontrol account route is unavailable upstream"
	result := map[string]any{
		...
		"provenance":   "live",
		"detail":       detail,
		"remediation":  "Retry later; do not treat this response as unauthenticated or as an empty account.",
	}
	if flags != nil && flags.asJSON {
		_ = json.NewEncoder(w).Encode(result)
	} else {
		fmt.Fprintf(w, "%s: %s (%s)\n", red("upstream_error"), endpoint, detail)
		fmt.Fprintln(w, "Retry later; do not treat this response as unauthenticated or as an empty account.")
	}
	if knownFragileAccountEndpoint(endpoint) {
		return nil
	}
	return apiErr(fmt.Errorf("%s at %s (%s)", message, endpoint, detail))
}

func knownFragileAccountEndpoint(endpoint string) bool {
	switch endpoint {
	case "/user/get_userid_details/", "/techmvc/mc_widgets/nudge_user_status/", "/my-watchlist":
		return true
	default:
		return false
	}
...
func knownFragileUpstreamError(err error) (detail, endpoint string, ok bool) {
	var apiErrValue *client.APIError
	if !errors.As(err, &apiErrValue) || apiErrValue.StatusCode < 500 {
		return "", "", false
	}
	endpoint = apiErrValue.Path
	if !knownFragilePublicEndpoint(endpoint) && !knownFragileAccountEndpoint(endpoint) {
		return "", "", false
	}
	return fmt.Sprintf("HTTP %d", apiErrValue.StatusCode), endpoint, true
}

func knownFragilePublicEndpoint(path string) bool {
	switch {
	case path == "/news/business/markets":
		return true
	case strings.HasPrefix(path, "/news/india/census-2027-rgi-issues-directives-for-formation-of-enumeration-blocks-14002256.html"):
		return true
	case path == "/techCharts/indianMarket/stock/history":
		return true
	case strings.HasPrefix(path, "/pricefeed/notapplicable/inidicesindia/"):
		return true
	case path == "/markets/commodities/", path == "/markets/currency/", path == "/mcapi/v1/stock/price-volume", path == "/markets/filings/":
		return true
	case strings.HasPrefix(path, "/pricefeed/nse/equitycash/"):
		return true
	default:
		return false
	}
...
func writeKnownFragileEnvelope(w io.Writer, flags *rootFlags, endpoint, detail string) {
	result := map[string]any{
		"error":        "Moneycontrol route unavailable upstream",
		"code":         5,
		"availability": "upstream_error",
		"stability":    "fragile",
		"endpoint":     endpoint,
		...
		"provenance":   "live",
		"detail":       detail,
		"remediation":  "Retry later; do not interpret this as an empty result.",
	}
	if flags != nil && flags.asJSON {
		_ = json.NewEncoder(w).Encode(result)
		return
	...
	fmt.Fprintf(w, "%s: %s (%s)\n", red("upstream_error"), endpoint, detail)
	fmt.Fprintln(w, "Retry later; do not interpret this as an empty result.")
}
...
// replacePathParamPreservingSlashes is for Moneycontrol path parameters whose
// value is itself a route (for example news/india/article-slug). Escape each
// segment while retaining the observed slash separators.
func replacePathParamPreservingSlashes(path, name, value string) string {
	segments := strings.Split(strings.Trim(value, "/"), "/")
	for i, segment := range segments {
		segments[i] = cliutil.EscapePathParam(segment)
	}
