...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  moneycontrol-pp-cli doctor
  moneycontrol-pp-cli doctor --json
  moneycontrol-pp-cli doctor --fail-on warn
  moneycontrol-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if !credentialConfigured {
					...
					report["auth_hint"] = "moneycontrol-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					...
					if cfg != nil && cfg.CredentialConfigured() {
						...
						report["auth_domain"] = "moneycontrol.com"
					}
				}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "moneycontrol-pp-cli", suggestion)
						} else {
...
func collectCredentialsLocationReport(report map[string]any, cfg *config.Config) {
	...
	remediationHint := "run auth login or auth logout"
