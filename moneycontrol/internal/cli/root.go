...
type rootFlags struct {
	...
	// invocation. Mirrors the MONEYCONTROL_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func shippedBinaryMode() bool {
	return os.Getenv("MONEYCONTROL_SHIPPED_BINARY") == "1"
}

func newRootCmd(flags *rootFlags) *cobra.Command {
	// Moneycontrol is a live read-through CLI. Keep local response caches,
	// learning journals, and local-store fallbacks disabled for every command;
	// credentials remain the only permitted persisted state.
	flags.noCache = true
	flags.dataSource = "live"

	rootCmd := &cobra.Command{
		Use:   "moneycontrol-pp-cli",
		Short: `Live Indian financial news, quotes and research context with explicit best-effort availability.`,
		Long: `Live Indian financial news, quotes and research context with explicit best-effort availability.
...
Health check: run 'moneycontrol-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("moneycontrol-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.noCache, "no-cache", true, "Response caching is disabled for live Moneycontrol reads")
	...
	rootCmd.PersistentFlags().StringVar(&flags.dataSource, "data-source", "live", "Data source for Moneycontrol reads (live only; local mirrors are disabled)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'moneycontrol-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if flags.dataSource != "live" {
			return usageErr(fmt.Errorf("--data-source=%s is disabled for Moneycontrol; use --data-source=live", flags.dataSource))
		}
	...
	rootCmd.AddCommand(newResearchCmd(flags))
	...
	// Local profiles, feedback, exports, tails, workflows, and learning
	// commands are intentionally omitted from the shipped binary. They rely
	// on local state, while Moneycontrol's product contract permits only the
	// secure credential/session store to persist data.
	if !shippedBinaryMode() {
	...
	rootCmd.AddCommand(newIndicesPromotedCmd(flags))
	rootCmd.AddCommand(newPriceVolumePromotedCmd(flags))
	rootCmd.AddCommand(newStocksPromotedCmd(flags))
	rootCmd.AddCommand(newTrendingPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCmd())
	if !shippedBinaryMode() {
		// Self-learning loop commands are available only to isolated unit-test
		// fixtures and development invocations, never to the shipped binary.
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCatalystsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelContextCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEventsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMarketCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelNewsCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c.DryRun = f.dryRun
	// Never allow a flag or profile to re-enable a Moneycontrol response cache.
	c.NoCache = true
