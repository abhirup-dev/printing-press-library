...
var whichIndex = []whichEntry{
	{Command: "context holdings", Description: "Join supplied broker holding symbols to live Moneycontrol quotes, tagged news, and event context.", Group: "Live portfolio context", WhyItMatters: "Use this when a broker already supplies holdings and the agent needs Moneycontrol narrative context without copy-paste."},
	{Command: "events upcoming", Description: "Show live filings, results, and corporate actions grouped for supplied companies.", Group: "Live event context", WhyItMatters: "Use this before a review when the agent must identify current company events from one source-limited call."},
	{Command: "market breadth", Description: "Combine live index snapshots with available breadth/change-table rows and report partial availability.", Group: "Live market synthesis", WhyItMatters: "Use this for a compact market-opening view when index movement and breadth context are needed together."},
	{Command: "catalysts", Description: "Filter live dated IPO, earnings, filing, and corporate-action pages by supplied symbols and time window.", Group: "Live event context", WhyItMatters: "Use this when an agent needs current catalysts around a watchlist but must not rely on cached articles."},
	{Command: "news timeline", Description: "Normalize a live company-tag page into URL-preserving chronological story rows.", Group: "Live news intelligence", WhyItMatters: "Use this when the question is what the current Moneycontrol tag page says about one company, not general market news."},
	{Command: "portfolio triage", Description: "Emit live headlines, timestamps, URLs, and event labels for supplied external holdings.", Group: "Live portfolio context", WhyItMatters: "Use this for a quick source-linked review of external holdings before deeper research."},
	{Command: "research packet", Description: "Assemble live quote, price-volume, company news, filings, results, and corporate-action context for one company.", Group: "Live research synthesis", WhyItMatters: "Use this when an agent needs a broad but bounded company snapshot rather than many uncoordinated endpoint calls."},
	{Command: "history", Description: "Historical or intraday OHLCV bars for an NSE symbol.", Group: "history", WhyItMatters: "Historical or intraday OHLCV bars for an NSE symbol."}, // pp:which-promoted
	{Command: "indices", Description: "Live quote for an Indian index.", Group: "indices", WhyItMatters: "Live quote for an Indian index."}, // pp:which-promoted
	{Command: "price-volume", Description: "Current and historical price-volume period data for a stock.", Group: "price-volume", WhyItMatters: "Current and historical price-volume period data for a stock."}, // pp:which-promoted
	{Command: "stocks", Description: "Full NSE equity quote for a stock by its moneycontrol sc_id.", Group: "stocks", WhyItMatters: "Full NSE equity quote for a stock by its moneycontrol sc_id."}, // pp:which-promoted
	{Command: "trending", Description: "Currently trending stocks on moneycontrol (best-effort; empty widgets are errors).", Group: "trending", WhyItMatters: "Currently trending stocks on moneycontrol (best-effort; empty widgets are errors)."}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  moneycontrol-pp-cli which "stale tickets"
  moneycontrol-pp-cli which "bottleneck"
  moneycontrol-pp-cli which --limit 1 "send message"
  moneycontrol-pp-cli which                                # list the full capability index`,
