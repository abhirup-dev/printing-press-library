...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MONEYCONTROL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MONEYCONTROL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "moneycontrol-pp-cli/feedback")
...
When ` + "`MONEYCONTROL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MONEYCONTROL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  moneycontrol-pp-cli feedback "docs said exclusive but the boundary is inclusive"
  moneycontrol-pp-cli feedback --stdin < notes.txt
  moneycontrol-pp-cli feedback list`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  moneycontrol-pp-cli feedback list
  moneycontrol-pp-cli feedback list --limit 5
  moneycontrol-pp-cli feedback list --json`,
