...
func Load(configPath string) (*Config, error) {
	...
	// Prefer the dedicated secure session store over legacy config/credentials
	// files. The store contains only browser-session material and its values
	// never appear in status metadata or command output.
	if material, backend, storeErr := ReadStoredAuthMaterial(); storeErr == nil {
		cfg.AccessToken = material.Cookie
		cfg.Headers = cloneStringMap(material.Headers)
		cfg.CredentialDomain = material.Domain
		cfg.AuthSource = "secure-store"
		cfg.CredentialSource = backend
	} else if !errors.Is(storeErr, ErrAuthNotFound) {
		return nil, fmt.Errorf("read secure auth store: %w", storeErr)
	}

	cfg.snapshotFileConfig()

	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(cliutil.EnvOverride("MONEYCONTROL_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
	...
	if v := cliutil.EnvOverride("MONEYCONTROL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("MONEYCONTROL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "moneycontrol-pp-cli", "config.json"), nil
}
