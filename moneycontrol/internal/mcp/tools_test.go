...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("MONEYCONTROL_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "moneycontrol-pp-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "moneycontrol-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("moneycontrol", "test")
	RegisterTools(s)
...
func TestMCPUnmigratedCheckpointsCannotProveCompletion(t *testing.T) {
	for _, cursor := range []string{"", "resume-here"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := db.SaveSyncStateAt("account", cursor, 0, time.Now().UTC()); err != nil {
				t.Fatal(err)
			}
...
func TestMCPSQLMissingStoreIsActionable(t *testing.T) {
	...
	if !strings.Contains(text, "populate") {
		t.Fatalf("missing-store error %q should tell the operator how to populate the store", text)
	}
}
...
func TestMCPSQLEmptyStoreReturnsActionableEnvelope(t *testing.T) {
	...
	if !strings.Contains(envelope.NextStep, "populate") {
		t.Fatalf("empty-store SQL next_step should tell the operator how to populate the store: %s", text)
	}
}
