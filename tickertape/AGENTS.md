# Tickertape Printed CLI Agent Guide

This directory is a generated `tickertape-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tickertape-pp-cli doctor --json
tickertape-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tickertape-pp-cli which "<capability>" --json
tickertape-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
tickertape-pp-cli <command> --agent
```
...
```bash
tickertape-pp-cli <command> --help
tickertape-pp-cli <command> --dry-run --agent
```
