...
var whichIndex = []whichEntry{
	{Command: "company scorecard", Description: "Show Tickertape score dimensions, locked fields, entry-point signals, and red flags without silently dropping premium data.", Group: "Transparent research", WhyItMatters: "Choose this when an agent needs transparent Tickertape opinion data and must distinguish locked from absent fields."},
	{Command: "company timeline", Description: "Combine live company events, news, results, and investor-presentation metadata into a dated source-labelled stream.", Group: "Transparent research", WhyItMatters: "Choose this for a current company-history pass before drilling into individual reports."},
	{Command: "lookup", Description: "Triages Indian stocks, mutual funds, ETFs, indices, and US securities through one live command while retaining source-specific identity fields.", Group: "Cross-asset triage", WhyItMatters: "Choose this when the asset class is unknown or an agent needs consistent first-pass research."},
	{Command: "inspect access", Description: "Explain source host, route, observation time, entity identity, lock state, and entitlement errors for any live result.", Group: "Transparent research", WhyItMatters: "Choose this when an agent must explain whether a result is public, locked, authenticated, or unavailable."},
	{Command: "market brief", Description: "Turn the live Market Mood Index payload into a concise timestamped brief of fear/greed, FII, momentum, volatility, and component signals.", Group: "Market context", WhyItMatters: "Choose this for current market context, not as a trading signal or historical series."},
	{Command: "company brief", Description: "Compose live company identity, summary, scorecard, reports, and access state into one bounded triage response.", Group: "Agent-native synthesis", WhyItMatters: "Choose this for the first pass on a company when raw endpoint-by-endpoint calls would waste agent context."},
	{Command: "screen catalog", Description: "Organize equity and mutual-fund filters, categories, universes, and prebuilt screens while preserving premium markers.", Group: "Agent-native synthesis", WhyItMatters: "Choose this to discover what can be screened before deciding whether a live query is safe and supported."},
	{Command: "homepage", Description: "Paginated homepage events with optional asset and type filters.", Group: "homepage", WhyItMatters: "Paginated homepage events with optional asset and type filters."}, // pp:which-promoted
	{Command: "search-resource", Description: "Suggest Indian assets and return search metadata.", Group: "search-resource", WhyItMatters: "Suggest Indian assets and return search metadata."}, // pp:which-promoted
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  tickertape-pp-cli which "stale tickets"
  tickertape-pp-cli which "bottleneck"
  tickertape-pp-cli which --limit 1 "send message"
  tickertape-pp-cli which                                # list the full capability index`,
