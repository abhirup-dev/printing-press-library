...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			...
			"\n      Run 'tickertape-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'tickertape-pp-cli doctor' to check connectivity.", err))
...
func filterFieldsChecked(data json.RawMessage, fields string) (json.RawMessage, error) {
	...
	valid := ""
	dryRunEnvelope := isDryRunJSON(data)
	...
	for i, path := range paths {
		...
		// Dry-run responses intentionally contain request metadata rather than
		// provider fields. Treat the conventional `data` selector as an
		// acceptable no-op so generated examples remain verify-friendly.
		if dryRunEnvelope && strings.EqualFold(requestedPaths[i], "data") {
			continue
		}
		if valid == "" {
...
func isDryRunJSON(data json.RawMessage) bool {
	var object map[string]json.RawMessage
	if err := json.Unmarshal(data, &object); err != nil {
		return false
	}
	var value bool
	return json.Unmarshal(object["dry_run"], &value) == nil && value
}
