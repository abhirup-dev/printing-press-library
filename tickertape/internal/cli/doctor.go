...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tickertape-pp-cli doctor
  tickertape-pp-cli doctor --json
  tickertape-pp-cli doctor --fail-on warn
  tickertape-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "tickertape-pp-cli", suggestion)
						} else {
...
func collectCacheReport(_ context.Context, _ string) map[string]any {
	return map[string]any{
		"status":  "disabled",
		"source":  "live-only",
		"message": "domain data is never cached or stored locally; each research command fetches live data",
	}
}
